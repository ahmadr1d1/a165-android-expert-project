...
class TourismRepository private constructor(
    ...
    override fun getAllTourism(): LiveData<Resource<List<Tourism>>> =
        object : NetworkBoundResource<List<Tourism>, List<TourismResponse>>(appExecutors) {
            override fun loadFromDB(): LiveData<List<Tourism>> {
                val source: LiveData<List<TourismEntity>> = localDataSource.getAllTourism()
                val result = MediatorLiveData<List<Tourism>>()

                result.addSource(source) { entities ->
                    // manual konversi (helper singkat bisa dilihat di fun getFavoriteTourism)
                    result.value = DataMapper.mapEntitiesToDomain(entities)
                }
                return result
            }
    ...
    override fun getFavoriteTourism(): LiveData<List<Tourism>> {
        return localDataSource.getFavoriteTourism().map {
            DataMapper.mapEntitiesToDomainManual(it)
        }
    }
